  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        head_row: "flex",
        head_cell: cn(
          "text-muted-foreground rounded-md w-8 sm:w-9 font-normal",
          isMobile ? "text-[0.7rem]" : "text-[0.8rem]"
        ),
        row: "flex w-full mt-1 sm:mt-2",
        cell: cn(
          isMobile ? "h-8 w-8 text-center text-xs p-0" : "h-9 w-9 text-center text-sm p-0",
      ...
      components={{
        IconLeft: ({ ..._props }) => <ChevronLeft className={isMobile ? "h-3 w-3" : "h-4 w-4"} />,
        IconRight: ({ ..._props }) => <ChevronRight className={isMobile ? "h-3 w-3" : "h-4 w-4"} />,
      }}
      {...props}
    />
  );
